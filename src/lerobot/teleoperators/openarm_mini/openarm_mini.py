...
# OpenArm Mini gripper reports 0-100 percent. OpenArmX follower send_action()
# receives degrees, then converts degrees to radians before RobStride MIT control.
# Use 0.6 degree/percent: 100% = 60 deg = 1.0472 rad. This is not the
# official OpenArm -65 deg mapping and not the ROS2/URDF 0-0.044 m joint position.
GRIPPER_TELEOP_TO_DEGREES = 0.6


def _clamp_gripper_percent(value: float) -> float:
...
class OpenArmMini(Teleoperator):
    ...
    def __init__(self, config: OpenArmMiniConfig):
        ...
        self._bus_calibration = self._calibration_for_bus()
    ...
    def calibrate(self) -> None:
        ...
        if self.calibration:
            ...
            if user_input.strip().lower() != "c":
                ...
                self.bus.write_calibration(self._calibration_for_bus())
                return
    ...
    def _calibration_for_bus(self) -> dict[str, MotorCalibration]:
        """Return calibration keys matching this single-arm bus.

        Older bimanual OpenArm Mini calibrations were stored with keys like
        `left_joint_1` and `right_joint_1`. The current implementation wraps two
        single-arm OpenArmMini instances, whose buses expect `joint_1`.
        """
        if not self.calibration:
            return {}

        bus_keys = set(self.bus.motors) if hasattr(self, "bus") else {
            "joint_1",
            "joint_2",
            "joint_3",
            "joint_4",
        ...
        if set(self.calibration).issubset(bus_keys):
        ...
        stripped = {
            key.removeprefix(prefix): cal for key, cal in self.calibration.items() if key.startswith(prefix)
        }
        if stripped:
            logger.info("Using %s-prefixed OpenArm Mini calibration for %s", self.config.side, self.id)
            return stripped
    ...
    def get_action(self) -> RobotAction:
        ...
        for motor, val in positions.items():
            ...
            if motor == "gripper":
                action[f"{target}.pos"] = _clamp_gripper_percent(val) * GRIPPER_TELEOP_TO_DEGREES
            else:
    ...
    def write_goal_positions(self, positions: dict[str, float]) -> None:
        ...
        for key, val in positions.items():
            ...
            if base == "gripper":
                goals[target] = _clamp_gripper_percent(val / GRIPPER_TELEOP_TO_DEGREES)
            else:
