...
class RobstrideMotorsBus(MotorsBusBase):
    ...
    def __init__(
        ...
        Args:
            ...
            can_interface: CAN interface type - "auto" (default), "socketcan", or "slcan".
            use_can_fd: Whether to use CAN FD mode (default: True for OpenArms)
            bitrate: Nominal bitrate in bps (default: 1000000 = 1 Mbps)
            data_bitrate: Data bitrate for CAN FD in bps, ignored if use_can_fd is False.
        """
        ...
        # Dynamic gains storage for the write/sync_write convenience path.
        self._gains: dict[str, dict[str, float]] = {}
        for name, motor in self.motors.items():
            motor_type_str = motor.motor_type_str or motor.model
            self._motor_types[name] = self._resolve_motor_type(motor_type_str)

            # Fixed gains at startup for every motor.
    ...
    def _resolve_motor_type(self, motor_type: str | None) -> MotorType:
        ...
        key = aliases.get(motor_type.upper(), motor_type.upper())
        return getattr(MotorType, key)
    ...
    def connect(self, handshake: bool = True) -> None:
        ...
        try:
            ...
            if self.can_interface == "socketcan" and self.use_can_fd and self.data_bitrate is not None:
                ...
                logger.info(
                    "Connected to %s with CAN FD (bitrate=%s, data_bitrate=%s)",
                    self.port,
                    self.bitrate,
                    self.data_bitrate,
                )
                ...
                logger.info(
                    "Connected to %s with %s (bitrate=%s)",
                    self.port,
                    self.can_interface,
                    self.bitrate,
                )
            ...
            # Let the adapter synchronize to the bus before sending frames.
            # PEAK USB adapters need a brief settling period after socket open.
            time.sleep(0.1)
    ...
    def _query_status_via_clear_fault(
        ...
        master_id = self._get_motor_master_id(motor_name)
        # 29-bit extended frame: arb = (STATUS_QUERY << 24) | (0x00 << 16) | (MASTER_ID << 8) | motor_id
        arb_id = (CAN_COMM_STATUS_QUERY << 24) | (master_id << 8) | motor_id
        data = [0x01] + [0x00] * 7
    ...
    def _recv_status_via_clear_fault(
        ...
        while time.time() - start_time < timeout:
            ...
            # Response arb_id format: (comm_type<<24) | (motor_id<<8) | master_id
            # Motor ID is at bits [15:8] of the 29-bit arb_id.
            rx_motor_id = (msg.arbitration_id >> 8) & 0xFF
            if expected_recv_id is not None and rx_motor_id != expected_recv_id:
                logger.debug(
                    "Discarding frame: arb=0x%08X data=%s (expected motor_id=0x%02X, got 0x%02X)",
                    msg.arbitration_id,
                    bytes(msg.data).hex(),
                    expected_recv_id,
                    rx_motor_id,
            ...
            # Any matching frame from the motor is a valid response.
            return False, msg
    ...
    def update_motor_state(self, motor: NameOrID) -> bool:
        ...
        self._decode_motor_state(msg.data, motor_id=motor_id)
        return True
    ...
    def _handshake(self) -> None:
        ...
        for motor_name in self.motors:
            ...
            if msg is None:
                ...
                try:
                    rx_motor_id = (msg.arbitration_id >> 8) & 0xFF
                    self._decode_motor_state(msg.data, motor_id=rx_motor_id)
                except Exception as e:
                    logger.debug(
    ...
    def _switch_operation_mode(self, motor: NameOrID, mode: ControlMode) -> None:
        ...
        NOTE: OpenArmX motors power on in MIT mode by default; this function
        is not used in the standard openarmx_follower connect flow.
        Mode switching via the SDK uses parameter-write frames (not yet implemented here).
        """
        motor_name = self._get_motor_name(motor)
    ...
    def _enable_motor(self, motor: NameOrID) -> None:
        """Enable a single motor (enter Motor mode)."""
        motor_id = self._get_motor_id(motor)
        recv_id = self._get_motor_recv_id(motor)
        master_id = self._get_motor_master_id(motor)
        arb_id = (CAN_COMM_ENABLE << 24) | (master_id << 8) | motor_id
    ...
    def _disable_motor(self, motor: NameOrID) -> None:
        """Disable a single motor (enter Reset mode)."""
        motor_id = self._get_motor_id(motor)
        recv_id = self._get_motor_recv_id(motor)
        master_id = self._get_motor_master_id(motor)
        arb_id = (CAN_COMM_DISABLE << 24) | (master_id << 8) | motor_id
        msg = can.Message(arbitration_id=arb_id, data=[0x00] * 8, is_extended_id=True)
        self._bus().send(msg)
    ...
    def set_zero_position(self, motors: str | list[str] | None = None) -> None:
        ...
        for motor in motors:
            ...
            recv_id = self._get_motor_recv_id(motor)
            master_id = self._get_motor_master_id(motor)
            arb_id = (CAN_COMM_SET_ZERO << 24) | (master_id << 8) | motor_id
            data = [0x01] + [0x00] * 7
            msg = can.Message(arbitration_id=arb_id, data=data, is_extended_id=True)
            self._bus().send(msg)
            self._recv_motor_response(expected_recv_id=recv_id)
    ...
    def _recv_motor_response(
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                ...
                if msg:
                    rx_motor_id = (msg.arbitration_id >> 8) & 0xFF
                    messages_seen.append(f"0x{rx_motor_id:02X}")
                    # If no filter specified, return any message
                    if expected_recv_id is None:
                        return msg
                    # Motor ID lives in arb_id bits [15:8]
                    if rx_motor_id == expected_recv_id:
                        return msg
                    else:
                        logger.debug(
                            "Ignoring frame arb=0x%08X motor_id=0x%02X, expected 0x%02X",
                            msg.arbitration_id,
                            rx_motor_id,
                            expected_recv_id,
                        )
            ...
            if logger.isEnabledFor(logging.DEBUG):
                if messages_seen:
                    logger.debug(
                        "Received %s frame(s) from motor IDs %s, but expected 0x%02X",
                        len(messages_seen),
                        set(messages_seen),
                        expected_recv_id,
                    )
                else:
                    logger.debug(f"No CAN messages received (expected motor_id 0x{expected_recv_id:02X})")
        except Exception as e:
    ...
    def _recv_all_responses(
        ...
        try:
            while len(responses) < len(expected_recv_ids) and (time.time() - start_time) < timeout:
                ...
                if msg:
                    motor_id = (msg.arbitration_id >> 8) & 0xFF
                    if motor_id in expected_set:
                        responses[motor_id] = msg
        ...
        for msg in messages:
            if len(msg.data) < 8:
                logger.debug(
                    ...
                    f"(arb=0x{int(msg.arbitration_id):08X}, data={bytes(msg.data).hex()})"
                )
                continue

            # Motor ID is in bits [15:8] of the 29-bit response arb_id.
            motor_id = (msg.arbitration_id >> 8) & 0xFF
            motor_name = self._id_to_name.get(motor_id)
            if motor_name is None:
                logger.debug(
                    f"Unmapped CAN frame on {self.port} "
                    f"(arb=0x{int(msg.arbitration_id):08X}, motor_id=0x{motor_id:02X}, "
                    f"data={bytes(msg.data).hex()})"
                )
                continue

            self._process_response(motor_name, msg)
            processed_recv_ids.add(motor_id)
    ...
    def _speed_control(
        ...
        if recv_id is not None:
            ...
            if resp:
                rx_motor_id = (resp.arbitration_id >> 8) & 0xFF
                self._decode_motor_state(resp.data, motor_id=rx_motor_id)
    ...
    def _mit_control(
        ...
        *,
        ...
        Args:
            ...
            kp: Position gain
            kd: Velocity gain
            position_rad: Target position (rad)
            velocity_rad_per_sec: Target velocity (rad/s)
        ...
        motor_id = self._get_motor_id(motor)
        data, tau_uint = self._encode_mit_packet(
            motor_type, kp, kd, position_rad, velocity_rad_per_sec, torque
        )
        # 29-bit arb: (MIT_CTRL << 24) | (torque_u16 << 8) | motor_id
        arb_id = (CAN_COMM_MIT_CTRL << 24) | (tau_uint << 8) | motor_id
        msg = can.Message(arbitration_id=arb_id, data=data, is_extended_id=True)
        self._bus().send(msg)
    ...
    def _encode_mit_packet(
        ...
        """Encode an MIT control packet for the 29-bit extended CAN protocol.

        Returns:
            (data, tau_uint) where data is the 8-byte payload and tau_uint
            is the 16-bit torque value that must be placed in the arb_id.

        OpenArmX protocol (all values 16-bit, big-endian):
          data[0:2] = position_uint16  scaled [-pmax, pmax]
          data[2:4] = velocity_uint16  scaled [-vmax, vmax]
          data[4:6] = kp_uint16        scaled [0, kp_max]
          data[6:8] = kd_uint16        scaled [0, kd_max]
          arb_id bits[23:8] = torque_uint16  scaled [-tmax, tmax]
        """
        pmax, vmax, tmax = MOTOR_LIMIT_PARAMS[motor_type]
        kp_max = MIT_KP_MAX[motor_type]
        kd_max = MIT_KD_MAX[motor_type]

        pos_uint = self._float_to_uint(position_rad, -pmax, pmax, 16)
        vel_uint = self._float_to_uint(velocity_rad_per_sec, -vmax, vmax, 16)
        tau_uint = self._float_to_uint(torque, -tmax, tmax, 16)
        kp_uint = self._float_to_uint(kp, 0.0, kp_max, 16)
        kd_uint = self._float_to_uint(kd, 0.0, kd_max, 16)

        data = [
            (pos_uint >> 8) & 0xFF, pos_uint & 0xFF,
            (vel_uint >> 8) & 0xFF, vel_uint & 0xFF,
            (kp_uint >> 8) & 0xFF, kp_uint & 0xFF,
            (kd_uint >> 8) & 0xFF, kd_uint & 0xFF,
        ]
        return data, tau_uint
    ...
    def _mit_control_batch(
        ...
        Command tuple layout is `(kp, kd, position_rad, velocity_rad_per_sec, torque)`.
        """
        if not commands:
            return

        recv_id_to_motor: dict[int, str] = {}
        for motor, (kp, kd, position_rad, velocity_rad_per_sec, torque) in commands.items():
            motor_name = self._get_motor_name(motor)
            if self.operation_mode[motor_name] != ControlMode.MIT:
                raise RuntimeError(f"Motor '{motor_name}' is not in MIT control mode.")
    ...
    def mit_control_batch(
        ...
        """Public radians-based MIT batch control API."""
    ...
    def _decode_motor_state(
        self, data: bytearray | bytes, motor_id: int | None = None
    ) -> tuple[float, float, float, float]:
        """Decode motor state from a 29-bit extended CAN response frame.

        OpenArmX protocol data layout (all 16-bit big-endian):
          data[0:2] = position_uint16   scaled [-pmax, pmax] rad
          data[2:4] = velocity_uint16   scaled [-vmax, vmax] rad/s
          data[4:6] = torque_uint16     scaled [-tmax, tmax] N·m
          data[6:8] = temperature*10    uint16 big-endian (°C × 10)

        Args:
            data: 8-byte CAN payload.
            motor_id: Motor ID extracted from response arb_id bits [15:8].
                      Required — the protocol no longer encodes it in data[0].
        """
        if len(data) < 8:
            raise ValueError("Invalid motor state data")
        if motor_id is None:
            raise ValueError("motor_id must be provided (extracted from arb_id >> 8)")

        motor_name = self._id_to_name.get(motor_id)
        if motor_name is None:
            raise ValueError(f"Unknown motor ID 0x{motor_id:02X}")

        motor_type = self._motor_types[motor_name]
        pmax, vmax, tmax = MOTOR_LIMIT_PARAMS[motor_type]

        q_uint = (data[0] << 8) | data[1]
        dq_uint = (data[2] << 8) | data[3]
        tau_uint = (data[4] << 8) | data[5]
        t_raw = (data[6] << 8) | data[7]   # temperature × 10 in °C

        position_rad = self._uint_to_float(q_uint, -pmax, pmax, 16)
        velocity_rad_per_sec = self._uint_to_float(dq_uint, -vmax, vmax, 16)
        torque = self._uint_to_float(tau_uint, -tmax, tmax, 16)
        temp_celsius = t_raw / 10.0

        self.last_feedback_time[motor_name] = time.time()
        self._last_known_states[motor_name] = {
            "position": position_rad,
            "velocity": velocity_rad_per_sec,
            "torque": torque,
            "temp_mos": temp_celsius,
            "temp_rotor": 0.0,
        }
        return position_rad, velocity_rad_per_sec, torque, temp_celsius
    ...
    def _process_response(self, motor: str, msg: can.Message) -> None:
        ...
        try:
            ...
            logger.warning(
                ...
                f"(arb=0x{int(msg.arbitration_id):08X}, data={bytes(msg.data).hex()}): {e}"
            )
    ...
    def read(
        ...
        """Read a value from a single motor. Positions are in radians."""
    ...
    def write(
        ...
        """Write a value to a single motor. Positions are in radians."""
        motor_name = self._get_motor_name(motor)

        if data_name in ("Kp", "Kd"):
            self._gains[motor_name][data_name.lower()] = float(value)
        elif data_name == "Goal_Position":
            # Use MIT control with position in radians
    ...
    def sync_write(
        ...
        """
        Write different values to multiple motors simultaneously. Positions are in radians.
        Uses batched operations where supported.
        """
        if data_name in ("Kp", "Kd"):
            key = data_name.lower()
            ...
            commands: dict[NameOrID, tuple[float, float, float, float, float]] = {}
            for motor, value_rad in values.items():
                motor_name = self._get_motor_name(motor)
                commands[motor] = (
    ...
    def record_ranges_of_motion(
        ...
        """
        Interactively record the min/max values of each motor in radians.
        ...
        # Get initial positions (already in radians)
        ...
        while not user_pressed_enter:
            ...
            if display_values:
                print("\n" + "=" * 50)
                print(
                    f"{'MOTOR':<20} | {'MIN (rad)':>12} | "
                    f"{'POS (rad)':>12} | {'MAX (rad)':>12}"
                )
                print("-" * 50)
                for motor in target_motors:
                    if motor in positions:
                        print(
                            f"{motor:<20} | {mins[motor]:>12.1f} | "
                            f"{positions[motor]:>12.1f} | {maxes[motor]:>12.1f}"
                        )
        ...
        # Validate ranges
        for motor in target_motors:
            if (motor in mins) and (motor in maxes) and (abs(maxes[motor] - mins[motor]) < 0.087):
                raise ValueError(f"Motor {motor} has insufficient range of motion (< 0.087 rad)")
    ...
    def _get_motor_master_id(self, motor: NameOrID) -> int:
        """Return the host/master ID used in RobStride management arbitration IDs."""
        motor_name = self._get_motor_name(motor)
        motor_obj = self.motors[motor_name]
        return int(getattr(motor_obj, "master_id", CAN_MASTER_ID))

    def _get_motor_recv_id(self, motor: NameOrID) -> int:
        """Return the expected motor ID found in response arbitration bits [15:8].

        Some callers set `motor.recv_id`; otherwise we fall back to the configured `motor.id`.
        """
        motor_name = self._get_motor_name(motor)
        motor_obj = self.motors[motor_name]
